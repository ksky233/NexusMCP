...
class InMemoryExecutionUnitOfWork:
    def __init__(
        ...
        *,
        ...
        self._stores = (execution_store, approval_store, audit_store)
        self._lock = lock
        self._working: (
            tuple[
    ...
    async def __aenter__(self) -> InMemoryExecutionUnitOfWork:
        ...
        execution_store, approval_store, audit_store = self._stores
        self._working = (
            dict(execution_store),
            dict(approval_store),
            dict(audit_store),
        )
        executions, approvals, audits = self._working
    ...
    async def commit(self) -> None:
        executions, approvals, audits = self._require_working()
        execution_store, approval_store, audit_store = self._stores
